...
def _compute_window_metrics(
    ...
    # baseline indices: first baseline_frac of window (at least 1 timepoint)
...
def _active_mask_from_metrics(
    ...
    if method == "max_z":
        return max_z >= float(z_thresh)

    if method == "auc":
        return auc >= float(auc_thresh)

    if method == "mean_over_baseline":
        return mean_over_baseline >= float(mean_thresh)

    if method == "prominence":
        return (prom >= float(prom_thresh)) & (n_peaks >= float(min_peaks))
...
def create_new_run(
    *,
    ...
    # root clustering on ACTIVE traces (windowed traces)
    state = {"clusters": {}}
    state = _cluster_and_update_state(X=Xw, active_idx=active_idx, state=state, prefix="", target_idx=None, k=k)

    cluster_excels = _export_cluster_excels(df_raw, state["clusters"], clusters_dir, run_id)
    cluster_plots = _write_cluster_plots(Xw, tw, state["clusters"], plots_dir, run_id)
...
def subclassify_selected(
    *,
    ...
    # recompute metrics + active mask to keep consistency with new window/method
    metrics = _compute_window_metrics(Xw, tw, baseline_frac=baseline_frac, min_peaks=min_peaks)
    active_mask = _active_mask_from_metrics(
        metrics=metrics,
        activity_method=activity_method,
    ...
    # update active/non-active exports
    clusters_dir = os.path.join(out_dir, "clusters")
    plots_dir = os.path.join(out_dir, "plots")
    os.makedirs(clusters_dir, exist_ok=True)
    os.makedirs(plots_dir, exist_ok=True)
    ...
    for key in selected_clusters:
        ...
        state2 = _cluster_and_update_state(
            X=Xw,
        ...
        clusters = state2["clusters"]

    cluster_excels = _export_cluster_excels(df_raw, clusters, clusters_dir, run_id)
    cluster_plots = _write_cluster_plots(Xw, tw, clusters, plots_dir, run_id)

    # metrics csv export (updated)
    metrics_df = pd.DataFrame({
        "row_index": np.arange(len(df_raw)),
        "active": active_mask.astype(int),
        "max_z": metrics["max_z"],
